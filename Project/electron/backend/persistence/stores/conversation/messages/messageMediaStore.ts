...
export class MessageMediaStore {
    ...
    async getDataUrl(mediaId: string): Promise<string | null> {
        const { db } = getPersistence();
        const row = await db
            .selectFrom('message_media')
            .select(['mime_type', 'bytes_blob'])
            .where('media_id', '=', mediaId)
            .executeTakeFirst();
        ...
        return `data:${row.mime_type};base64,${Buffer.from(row.bytes_blob).toString('base64')}`;
    }

    async getDataUrlForProfile(profileId: string, mediaId: string): Promise<string | null> {
        const { db } = getPersistence();
        const row = await db
            .selectFrom('message_media')
            ...
            .select(['message_media.mime_type as mime_type', 'message_media.bytes_blob as bytes_blob'])
